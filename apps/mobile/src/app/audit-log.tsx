...
export default function AuditLogScreen() {
  ...
  }, [fromDate, actionFilter]);

  const { data: actionTypes } = trpc.audit.actionTypes.useQuery({}, { retry: false });

  const { data, isLoading, isFetching, error } = trpc.audit.list.useQuery({
    limit: PAGE_SIZE,
    cursor,
    fromDate,
  ...
  // Merge data into items when query resolves
  useEffect(() => {
    if (!data) return;
    ...
    if (wasPaging) {
      setItems((prev) => {
        ...
        const newItems = (data.items as AuditItem[]).filter((i) => !ids.has(i.id));
