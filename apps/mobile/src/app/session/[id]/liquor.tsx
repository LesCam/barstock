...
export default function LiquorWeighScreen() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (matchedTemplate && selectedItem.barcode && grossWeightG != null) {
        ...
        if (tareG != null) {
          ...
          if (Math.abs(grossWeightG - tareG) <= threshold) {
            const capturedBarcode = selectedItem.barcode;
            const capturedWeight = grossWeightG;
            const capturedContainerMl = Number(matchedTemplate.containerSizeMl) || undefined;
            Alert.alert(
                ...
                { text: "No Thanks", style: "cancel" },
                {
                  ...
                  onPress: () => {
                    contributeTareMutation.mutate({
                      ...
                      measuredWeightG: capturedWeight,
                      sourceType: "empty_confirmed",
                      sessionId: sessionId!,
                      containerSizeMl: capturedContainerMl,
                    });
                  },
                },
              ]
                ...
                <Text style={styles.hintText}>
                  {[
                    ...
                    hint.lastCountValue != null && hint.avgDailyUsage != null && hint.avgDailyUsage > 0
                      ? `Est: ~${Math.round(Math.max(0, hint.lastCountValue - hint.avgDailyUsage * ((Date.now() - new Date(hint.lastCountDate!).getTime()) / 86400000)) * 10) / 10}${hint.isWeight ? "g" : ""}`
                      : null,
                  ].filter(Boolean).join(" · ")}
                </Text>
              )}
